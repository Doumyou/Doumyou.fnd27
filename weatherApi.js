...
async function getWeather(){
    ...
    //初期化
    const timeDefinesList = [];     //日付と天気アドレスを入れるID用
    const weatherCodeList = [];     //各日の天気コード
    let getYearMonth;               //取得したデータの年・月
    let day1;

    await fetch(url)
        //対象のURLから取得したデータをJSON形式のJavaScriptオブジェクトに変換
        .then(function (response) {
            return response.json();
        })
        //取得したデータから、天気情報の抽出
        .then(function (weather) {
            const weatherCodes = weather[0].timeSeries[0].areas[0].weatherCodes;        //天気コード
            const timeDefines = weather[0].timeSeries[0].timeDefines;                   //時間の定義
            //1～2日目のデータをそれぞれの配列に入れる
            weatherCodeList.push(weatherCodes[0], weatherCodes[1]);                     //1週間の天気コード
            timeDefinesList.push(timeDefines[0], timeDefines[1]);                       //1週間の取得した日時
            //3日目以降のデータをそれぞれの配列に追加
            ...
            let currentTime = new Date(timeDefinesList[0]);
            getYearMonth = String(currentTime.getFullYear()) + String(currentTime.getMonth()+1);
            for (let i=0; i < timeDefinesList.length; i++){
                currentTime = new Date(timeDefinesList[i]);
                timeDefinesList[i] = "weather" + String(currentTime.getFullYear()) + String(currentTime.getMonth()+1) + String(currentTime.getDate());
            }
        })
        .then(function(){   //カレンダーの表示月が取得したAPIの取得月と同じであればIDを入れる
            //年月を比較して同じであればデータの反映を行う
            let currentYear = String(parseInt(selectYear.value));      //表示しているカレンダーの年を取得
            let currentMonth = String(parseInt(selectMonth.value)+1);      //表示しているカレンダーの月を取得
            if (currentYear + currentMonth === getYearMonth){
                for (let i = 0; i < weatherCodeList.length; i++){
                    document.getElementById(timeDefinesList[i]).innerHTML = "<img src=\"" + "https://www.jma.go.jp/bosai/forecast/img/" + weatherCode[weatherCodeList[i]][0] + "\" alt=\"weather\" width=\"30\" height=\"30\">";
                }
            }
        });
}
